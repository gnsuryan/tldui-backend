...
consecutive_test_failure_report_sql_query=`select to_char(consecutive_days) consecutive_days,team, run_key,test_unit,test_case_logical_name,test_path,release,risk_category from
(
            Select
            ...
            (
                ...
                (select distinct build_label from wlsqa.test_run wtr where wtr.release='12.2.1.5.0' and wtr.run_type like 'nightly%' order by wtr.build_label desc fetch first 7 rows only)
...
union all
select to_char(consecutive_days) consecutive_days,team, run_key,test_unit,test_case_logical_name,test_path,release,risk_category from
(
            Select
            team, release,run_key,risk_category,test_unit,test_case_logical_name,test_path,
            wlsqa.get_consecutive_builds_test_fail_ct(release,team,run_key,risk_category,test_unit,test_case_logical_name,test_path,7,FAIL_COUNT)
            ...
            (
                ...
                and trs.test_status in ('FAILURE','SKIP')
                and tr.build_label in
                (select distinct build_label from wlsqa.test_run wtr where wtr.release='14.1.2.0.0' and wtr.run_type like 'nightly%' order by wtr.build_label desc fetch first 7 rows only)
                group by tr.team, tr.release,tr.run_key,tr.risk_category,trs.test_unit,trs.test_case_logical_name,trs.test_path
                having count(*) > 1
            )
) where consecutive_days > 1`
